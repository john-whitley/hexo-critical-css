 ...
 */
function applyCriticalToFile(publicDir, htmlFile, log) {
  ...
  var criticalPromise = Critical.generate({
    ...
    var toReplace = /<\/\s*head>/;
    var replacement = '<style type="text/css">' + criticalCss + '</style></head>';
...
function CriticalCssWorker() {
  var log = this.log;
  var options = this.config.csscrticial;
  ...
  return fs.exists(publicDir).then(function (exist) {
    ...
    htmlFiles.forEach(function(htmlFile) {
      var criticalPromise = applyCriticalToFile(publicDir, htmlFile, log);
