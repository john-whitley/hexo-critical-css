...
function getCriticalOptions(publicDir, htmlFile, options, log) {
  ...
  if (options.perPageCss) {
    var fullyQualifiedFilename = pathFn.join(publicDir, htmlFile);

    var filename = pathFn.basename(fullyQualifiedFilename);
    var base = fullyQualifiedFilename.replace(filename, '');
